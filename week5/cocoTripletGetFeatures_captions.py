...
def cache_outputs_coco_captions(loader, text_pre_model, modelText,  img_pre_model, modelImage, cache_filename, device):
    ...
    with torch.no_grad():
        ...
        for data, _ in tqdm(loader):
            data, captions = data
            start = time.time()
            captions = [text_pre_model.get_sentence_vector(re.sub(r'[^a-zA-Z ]', '', sentence[0].lower())) for sentence in captions]
            captions = torch.tensor(captions, device="cuda")
