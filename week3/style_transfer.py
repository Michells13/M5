...
if __name__ == "__main__":
    ...
    dataset = r"/ghome/group04/week3/datasets/dataset/"
    textures = r"/ghome/group04/week3/datasets/textures/"

    for dataset_filename in os.listdir(dataset):
        image = cv2.imread(os.path.join(dataset, dataset_filename))
        if image is not None:
            for texture_filename in os.listdir(textures):
                texture = cv2.imread(os.path.join(textures, texture_filename))
                if texture is not None:
                
                    content_image_path = dataset + dataset_filename
                    texture_path = textures + texture_filename
                    ...
                    style_tensor = preprocess_image(texture_path, result_height, result_width)
                    ...
                    name = "result/result_%s_%s.png" % (dataset_filename, texture_filename)
