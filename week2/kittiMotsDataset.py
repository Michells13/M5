...
def read_png_KITTI_MOTS(png_dir, pretrained, seg):
    """
...
def get_KITTI_MOTS_dataset_bbox(img_dir, annot_dir, pretrained = True):
    """
    This functions generates the dataset in COCO format given the path of the
    KITTI-MOTS dataset images and annotations (only with bboxes)
    ...
    for seq in seqs:
        ...
        for image in images:
            ...
            objs = read_png_KITTI_MOTS(annotFile, pretrained, seg = False)
...
def get_KITTI_MOTS_dataset(img_dir, annot_dir, pretrained = True):
    """
    This functions generates the dataset in COCO format given the path of the
    KITTI-MOTS dataset images and annotations (bboxes  + instance seg.)
    ...
    for seq in seqs:
        ...
        for image in images:
            ...
            # Read annotation PNG
            annotFile = os.path.join(annot_dir, seq, image)
            objs = read_png_KITTI_MOTS(annotFile, pretrained, seg = True)
